...
export default function LandingPage() {
  return (
    <div className="mesh-bg min-h-screen overflow-x-hidden">
      {/* ── NAVBAR ── */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-4 border-b border-white/5 bg-slate-950/60 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-lg shadow-indigo-500/30">
            <Sparkles className="w-5 h-5 text-white" />
          </div>
          <span className="text-xl font-bold text-white tracking-tight">ClubOps <span className="gradient-text">AI</span></span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/login"
            className="hidden md:block text-sm text-slate-400 hover:text-white transition-colors px-4 py-2"
          >
            Sign in
          </Link>
          <Link
            to="/register"
            id="nav-get-started"
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all hover:shadow-lg hover:shadow-indigo-500/25"
          >
      ...
      <section className="pt-32 pb-20 px-6 relative">
        ...
        <div className="max-w-6xl mx-auto text-center relative">
          {/* Badge */}
          <div className="animate-slide-up inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-semibold px-4 py-2 rounded-full mb-8 uppercase tracking-widest">
            <Sparkles className="w-3.5 h-3.5" />
            AI-Powered Club Management
          </div>

          {/* Headline */}
          <h1 className="animate-slide-up delay-100 text-5xl md:text-7xl font-black text-white leading-[1.08] tracking-tight mb-6">
            Run your club<br />
            <span className="gradient-text">10× smarter</span>
          </h1>

          {/* Sub-headline */}
          <p className="animate-slide-up delay-200 text-lg md:text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
          ...
          {/* CTAs */}
          <div className="animate-slide-up delay-300 flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
            <Link
              to="/register"
              id="hero-start-free"
              className="group flex items-center gap-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold px-8 py-4 rounded-2xl transition-all shadow-2xl shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-0.5"
            >
            ...
            <Link
              to="/login"
              className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold px-8 py-4 rounded-2xl transition-all"
            >
          ...
          {/* Hero UI Preview */}
          <div className="animate-fade-in delay-400 relative max-w-4xl mx-auto">
            {/* Floating cards around preview */}
            <FloatingCard className="absolute -left-4 md:-left-16 top-8 w-52 animate-float z-10 hidden md:block">
            ...
            <FloatingCard className="absolute -right-4 md:-right-16 top-12 w-48 animate-float-delay z-10 hidden md:block">
              <div className="flex items-center gap-2 mb-3">
            ...
            <div className="glass rounded-3xl overflow-hidden shadow-2xl shadow-black/60 border border-white/10">
              ...
              <div className="bg-slate-900/80 border-b border-white/5 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-bold text-sm">ClubOps AI</span>
                </div>
              ...
              <div className="flex h-72 md:h-80">
                {/* Mock sidebar */}
                <div className="w-14 md:w-48 bg-slate-900/60 border-r border-white/5 p-3 flex flex-col gap-1 shrink-0">
                  {[
                    ...
                    <div key={item.label} className={`flex items-center gap-3 px-2 md:px-3 py-2 rounded-xl transition-colors ${item.active ? 'bg-indigo-500/15 border border-indigo-500/20' : ''}`}>
                      <span className="text-sm">{item.icon}</span>
                      <span className={`hidden md:block text-xs font-medium ${item.active ? 'text-indigo-300' : 'text-slate-500'}`}>{item.label}</span>
                    </div>
                  ))}
                  <div className="mt-auto flex items-center gap-3 px-2 md:px-3 py-2 rounded-xl bg-violet-500/10 border border-violet-500/20">
                ...
                <div className="flex-1 p-5 overflow-hidden">
                  ...
                  {/* Event list mock */}
                  <p className="text-xs font-semibold text-slate-400 mb-3">Recent Events</p>
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={ev.title} className="flex items-center justify-between bg-slate-800/30 rounded-lg px-3 py-2 border border-white/5">
                        <div>
                          <p className="text-xs font-medium text-white">{ev.title}</p>
                          <p className="text-[10px] text-slate-500">{ev.date}</p>
                        </div>
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${ev.color}`}>{ev.status}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section id="stats" className="py-16 px-6">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {STATS.map(stat => (
            <div key={stat.label} className="text-center glass rounded-2xl py-8 px-4">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((feature, i) => (
              <div
                key={feature.title}
                className="group glass rounded-3xl p-6 hover:border-indigo-500/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl"
                style={{ animationDelay: `${i * 0.1}s` }}
              >
                <div
                  className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${feature.color} flex items-center justify-center mb-5 shadow-lg group-hover:scale-110 transition-transform`}
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="glass rounded-3xl p-8 md:p-12 border border-violet-500/20 relative overflow-hidden">
            {/* bg glow */}
            <div className="absolute inset-0 bg-gradient-to-br from-violet-600/10 to-indigo-600/5 pointer-events-none" />
            <div className="relative flex flex-col md:flex-row items-center gap-10">
              <div className="flex-1">
                <div className="inline-flex items-center gap-2 bg-violet-500/15 border border-violet-500/30 text-violet-300 text-xs font-semibold px-3 py-1.5 rounded-full mb-5 uppercase tracking-wider">
                  <Bot className="w-3.5 h-3.5" />
                  AI in action
                </div>
                <h2 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight">
                  Just type what<br />you need
                </h2>
                ...
                <Link
                  to="/register"
                  className="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-500 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-lg shadow-violet-500/30"
                >
                ...
                <div className="bg-slate-900/80 rounded-2xl border border-white/10 overflow-hidden">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="flex justify-end">
                      <div className="bg-indigo-600 text-white text-xs rounded-2xl rounded-tr-sm px-4 py-2.5 max-w-[85%]">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="bg-slate-800 text-slate-200 text-xs rounded-2xl rounded-tl-sm px-4 py-2.5 max-w-[85%] leading-relaxed">
                    ...
                    <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3">
                      <p className="text-[10px] text-slate-500 mb-2">Proposal #42 — 3 tasks → Arjun</p>
                      <div className="flex gap-2">
                        <button className="flex-1 bg-emerald-600/80 text-white text-[11px] font-semibold py-1.5 rounded-lg">✓ Apply</button>
                        <button className="flex-1 bg-slate-700 text-slate-300 text-[11px] font-semibold py-1.5 rounded-lg">✗ Reject</button>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="glass rounded-3xl p-12 relative overflow-hidden border border-indigo-500/20">
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  to="/register"
                  id="cta-register"
                  className="group flex items-center justify-center gap-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold px-8 py-4 rounded-2xl transition-all shadow-2xl shadow-indigo-500/30 hover:-translate-y-0.5"
                >
                  <Zap className="w-5 h-5" />
                  Create free account
                  <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
