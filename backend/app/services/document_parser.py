...
def extract_text(file_path: str, file_type: str = "txt") -> str:
    """Unified extract text returning all text as string."""
    ft = file_type.upper().lstrip(".")
    if ft == "PDF":
...
def chunk_text(
    ...
    """
    Chunks extracted sections or string into overlapping token/character windows while
    preserving exact page_number and section_name metadata.
    """
    if isinstance(sections, str):
        # Character overlap chunking for string input
        text = sections
        if not text:
            return []
        chunks = []
