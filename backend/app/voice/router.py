...
async def voice_rag_query(
    ...
    try:
        ...
        # 3. Text to Speech of the answer (synthesizes concise speech)
        speech_text = rag_resp.answer[:400] if len(rag_resp.answer) > 400 else rag_resp.answer
        tts_res = None
        try:
            tts_res = await _tts_service.synthesize_speech(
                text=speech_text,
        ...
        try:
            history_record = RAGChatHistory(
                ...
                citations=[c.model_dump() for c in rag_resp.citations],
        ...
        raise HTTPException(
            status_code=500,
            detail={"code": "VOICE_RAG_ERROR", "message": f"Voice RAG query failed: {str(e)}"}
        )
